...
| R-003 | Arquivos grandes ou lotes provocarem custo de API inesperado | M | A | um upload/item ativo; chunks confirmados não repetidos; estado ambíguo exige confirmação; contagens visíveis | produto | gate SEC pendente |
...
| R-013 | Divisão ou paralelismo de vídeos longos gerar lacunas, duplicações, texto fora de ordem ou timestamps incorretos | M | A | corte preferencial em silêncio, overlap de 2 s, offsets globais, merge/deduplicação determinísticos e um upload ativo | técnico | ADR/vetores propostos; gate pendente |
| R-014 | Checkpoint de retomada expor conteúdo ou virar histórico persistente | M | A | manifesto sem path/texto, resultado DPAPI, áudio não persistido, TTL 7 dias e cleanup | privacidade + técnico | gate SEC pendente |
