...
| R-002 | Conteúdo pessoal, confidencial ou protegido ser enviado à OpenAI sem compreensão do usuário | M | A | enviar somente o áudio do arquivo/segmento escolhido; aviso inicial aprovado e indicador cloud persistente; sem histórico ou telemetria | produto + privacidade | controles aprovados; implementação pendente |
| R-003 | Arquivos grandes ou lotes provocarem custo de API inesperado | M | A | um upload/item ativo; chunks confirmados não repetidos; estado ambíguo exige confirmação; contagens visíveis | produto | controles aprovados |
...
| R-005 | Falha de um item interromper o lote ou perder resultados concluídos | M | A | estado por item; fila avança; escrita atômica; retry somente de falhas | técnico | spec proposta |
...
| R-013 | Divisão ou paralelismo de vídeos longos gerar lacunas, duplicações, texto fora de ordem ou timestamps incorretos | M | A | corte preferencial em silêncio, overlap de 2 s, offsets globais, merge/deduplicação determinísticos e um upload ativo | técnico | estratégia aprovada; implementação pendente |
| R-014 | Checkpoint de retomada expor conteúdo ou virar histórico persistente | M | A | manifesto sem path/texto, resultado DPAPI, áudio não persistido, TTL 7 dias e cleanup | privacidade + técnico | controles/residual aprovados; implementação pendente |
...
- R-014 impede persistir resultados parciais antes de o owner aprovar retenção, proteção e residual.
