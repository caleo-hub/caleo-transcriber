...
$lintImports = Join-Path $projectRoot ".venv\Scripts\lint-imports.exe"
...
try {
    Invoke-Checked $python -m ruff format --check src tests
    Invoke-Checked $python -m ruff check src tests
    Invoke-Checked $python -m mypy src tests
    Invoke-Checked $lintImports
